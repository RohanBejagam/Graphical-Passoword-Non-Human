...
#register
def register(selected_image,selected_password, selected_name):
...
def create_registration_canvas(window):
    ...
    label = Label(registration_frame, text="Registration Page", font=("Ariel 15 bold"))
    canvas.create_window(550,40,anchor="nw",window=label)
    
    # Registration form labels
    user_label = Label(registration_frame, text="User name:", font=("Ariel 12 bold"))
    canvas.create_window(480, 130, anchor="nw", window=user_label)

    password_label = Label(registration_frame, text="Password:", font=("Ariel 12 bold"))
    canvas.create_window(480, 210, anchor="nw", window=password_label)

    image_label = Label(registration_frame, text="Image:", font=("Ariel 12 bold"))
    canvas.create_window(480, 290, anchor="nw", window=image_label)
    
    # Registration form entry fields
    user_entry = Entry(registration_frame, font=("Ariel 12"))
    user_entry.focus()
    canvas.create_window(580, 130, anchor="nw", window=user_entry)

    pas = StringVar()
    password_entry = Entry(registration_frame, textvar=pas, font=("Ariel 12"), show="*")
    ...
    # image_entry = Entry(registration_frame, font=("Ariel 12"))
    # canvas.create_window(580, 290, anchor="nw", window=image_entry)

    # Registration button
    custom_button.TkinterCustomButton(master=registration_frame, text="Register", height=40, corner_radius=10,
                                      command=lambda: register(s_image[0], password_entry.get(), user_entry.get())).place(relx=0.5, rely=0.7, anchor=CENTER)
